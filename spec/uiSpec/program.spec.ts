...
test('Program details', async ({ baseURL, page }) => {
    ...
    await page.getByRole('textbox', { name: 'Search all programs by name' }).fill('University of Washington - Seattle Master of Professional Accounting');
    await page.waitForTimeout(2000);
    const programNameRaw = await page.locator("(//div[@data-align='start'])[7]").allInnerTexts();
    const programTypeRaw = await page.locator("(//div[@data-align='start'])[8]").allInnerTexts();
    const durationRaw = await page.locator("(//div[@data-align='start'])[9]").allInnerTexts();
    // const tuitionRaw = await page.locator("(//div[@data-align='start'])[10]").allInnerTexts();
    const tuitionRaw = await page.locator("(//div[@data-align='start'])[11]").allInnerTexts(); // string[]
    const modalityRaw = await page.locator("(//div[@data-align='start'])[12]").allInnerTexts();
    const locationRaw = await page.locator("(//div[@data-align='start'])[13]").allInnerTexts();
    const timeCommitmentRaw = await page.locator("(//div[@data-align='start'])[14]").allInnerTexts();
